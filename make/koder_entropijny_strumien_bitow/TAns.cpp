...
void TAns::Code(int iP1, int s)
{
      ...
      while (m_iXstate >= m_iUs)
      {
       ...
       // cerr << m_iXstate % m_ib << " ";
        m_iXstate = floor(m_iXstate / m_ib);
      }
      if (s == 0) m_iXstate = floor(((m_iXstate + 1) * m_iL - 1) / (m_iL - iP1));
      else m_iXstate = floor((m_iXstate * m_iL / iP1));
}

void TAns::Decode(int iP1)
{
 
  m_iS = floor(((m_iXstate + 1) * iP1  - 1) / m_iL) -floor((m_iXstate * iP1  - 1) / m_iL); //czy jednka musi byc wewnatrz czy na zewn¹trz flora
